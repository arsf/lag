...
class LAScriterionClipBox : public LAScriterion
{
public:
  ...
  LAScriterionClipBox(F64 min_x, F64 min_y, F64 min_z, F64 max_x, F64 max_y, F64 max_z) { this->min_x = min_x; this->min_y = min_y; this->min_z = min_x; this->max_x = max_x; this->max_y = max_y; this->max_z = max_x; };
private:
...
class LAScriterionClipZBetween : public LAScriterion
{
public:
  inline const char* name() const { return "clip_z_between"; };
  inline int get_command(char* string) const { return sprintf(string, "-%s %g %g ", name(), below_z, above_z); };
  inline BOOL filter(const LASpoint* point) { return ((point->get_z() >= below_z) && (point->get_z() < above_z)); };
  LAScriterionClipZBetween(F64 below_z, F64 above_z) { this->below_z = below_z; this->above_z = above_z; };
private:
  F64 below_z, above_z;
};
...
class LAScriterionClipRawZBetween : public LAScriterion
{
public:
  inline const char* name() const { return "clip_raw_z_between"; };
  inline int get_command(char* string) const { return sprintf(string, "-%s %d %d ", name(), below_z, above_z); };
  inline BOOL filter(const LASpoint* point) { return ((point->z >= below_z) && (point->z < above_z)); };
  LAScriterionClipRawZBetween(I32 below_z, I32 above_z) { this->below_z = below_z; this->above_z = above_z; };
private:
  I32 below_z;
  I32 above_z;
};
...
public:
  ...
  inline int get_command(char* string) const { return sprintf(string, "-%s ", name()); };
  inline BOOL filter(const LASpoint* point) { return (point->classification & (1<<5)); };
};
...
public:
  ...
  inline int get_command(char* string) const { return sprintf(string, "-%s ", name()); };
  inline BOOL filter(const LASpoint* point) { return ((point->classification & (1<<5)) == 0); };
};
...
public:
  ...
  inline int get_command(char* string) const { return sprintf(string, "-%s ", name()); };
  inline BOOL filter(const LASpoint* point) { return (point->classification & (1<<6)); };
};
...
public:
  ...
  inline int get_command(char* string) const { return sprintf(string, "-%s ", name()); };
  inline BOOL filter(const LASpoint* point) { return ((point->classification & (1<<6)) == 0); };
};
...
public:
  ...
  inline int get_command(char* string) const { return sprintf(string, "-%s ", name()); };
  inline BOOL filter(const LASpoint* point) { return (point->classification & (1<<7)); };
};
...
public:
  ...
  inline int get_command(char* string) const { return sprintf(string, "-%s ", name()); };
  inline BOOL filter(const LASpoint* point) { return ((point->classification & (1<<7)) == 0); };
};
...
public:
  ...
  inline int get_command(char* string) const { return sprintf(string, "-%s %d %d ", name(), below_user_data, above_user_data); };
...
public:
  ...
  inline int get_command(char* string) const { return sprintf(string, "-%s %d ", name(), below_user_data); };
...
public:
  ...
  inline int get_command(char* string) const { return sprintf(string, "-%s %d ", name(), above_user_data); };
...
void LASfilter::usage() const
{
  ...
  fprintf(stderr,"  -clip_z_between 11.125 130.725 (min_z max_z)\n");
...
BOOL LASfilter::parse(int argc, char* argv[])
{
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    else if (strcmp(argv[i],"-clip_box") == 0 || strcmp(argv[i],"-clip_xyz") == 0)
    {
      ...
      add_criterion(new LAScriterionClipBox(atof(argv[i+1]), atof(argv[i+2]), atof(argv[i+3]), atof(argv[i+4]), atof(argv[i+5]), atof(argv[i+6])));
    ...
    else if (strcmp(argv[i],"-clip_z_between") == 0)
    {
      if ((i+2) >= argc)
      {
        fprintf(stderr,"ERROR: '%s' needs 2 arguments: min_z max_z\n", argv[i]);
      ...
      add_criterion(new LAScriterionClipZBetween(atof(argv[i+1]), atof(argv[i+2])));
      *argv[i]='\0'; *argv[i+1]='\0'; *argv[i+2]='\0'; i+=2;
    }
    ...
    else if (strcmp(argv[i],"-clip_raw_z_between") == 0)
    {
      if ((i+2) >= argc)
      {
        fprintf(stderr,"ERROR: '%s' needs 2 arguments: min_raw_z max_raw_z\n", argv[i]);
      ...
      add_criterion(new LAScriterionClipRawZBetween(atoi(argv[i+1]), atoi(argv[i+2])));
      *argv[i]='\0'; *argv[i+1]='\0'; *argv[i+2]='\0'; i+=2;
    }
...
{
  add_criterion(new LAScriterionClipBox(min_x, min_y, min_z, max_x, max_y, max_z));
}
