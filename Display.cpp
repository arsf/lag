...
bool Display::on_expose_event(GdkEventExpose* event){
   if(configuring){//Draw from scratch if window resized so that there is not (usually; this does not work perfectly) a blank area resulting.
      drawviewable(1);
      configuring = false;
   }
   else drawviewable(3);//Call this method with 3, which means that it is an expose call that this method will handle in a way depending on the subclass.
   return true;
}

//Please note that the Gtk::GL::DrawingArea::on_realize() method calls this event and is (by necessity) before the prepare_image() method, so it is necessary to be careful about what to put into this method as some things might crash if done before prepare_image().
...
bool Display::on_configure_event(GdkEventConfigure* event){
  ...
  resetview();
  configuring = true;//The expose event handler will be called immediately after the return statement, so tell it that the window has changed shape/size now.
  return true;
}
...
void Display::coloursandshades(double maxz,double minz,int maxintensity,int minintensity){
   ...
   colourheightarray = new double[30*(int)(rmaxz-rminz+4)];//This is at 30, rather than three, for a reason: three, one for each colour, by ten, so that the "resolution" of the colouring by height is 0.1 metres, not one whole metre. It makes colouring by height look better.
   ...
   brightnessheightarray = new double[10*(int)(rmaxz-rminz+4)];//This is at 30, rather than three, for a reason: three, one for each colour, by ten, so that the "resolution" of the colouring by height is 0.1 metres, not one whole metre. It makes shading by height look better.
...
void Display::colour_by(double value,double maxvalue,double minvalue,double& col1,double& col2,double& col3){//Following comments assume col1=red, col2=green and col3=blue.
  ...
  if(col1<0.0)col1=0.0;//These prevent the situation where a non-white colour can be made paler (rather than simply brighter) as a result of its RGB values being multiplied by some number greater than 1. Otherwise, non-white colours can eventually be made white when brightness mode is on.
...
double Display::brightness_by(double value,double maxvalue,double minvalue,double offsetvalue,double floorvalue){
  ...
  if(multiplier > 1)multiplier = 1;//This prevents the situation where a non-white colour can be made paler (rather than simply brighter) as a result of its RGB values being multiplied by some number greater than 1. Otherwise, non-white colours can eventually be made white when brightness mode is on.
  return multiplier;
}

//Convenience code for getting a subset and checking to see if the data is valid/useful.
bool Display::advsubsetproc(vector<PointBucket*>*& pointvector,double *xs,double *ys,int ps){
   ...
   if(pointvector==NULL||pointvector->size()==0){ return false; }//Do not put any output here as the user will be spammed horrendously because pointvector->size()==0 happens often (say if the user zooms in on an empty area).
   else return true;
}
...
//Convenience code called so that graphical artefacts through changes of view do not occur. This is because of being a multiwindow application.
void Display::guard_against_interaction_between_GL_areas(){
   get_gl_window()->make_current(get_gl_context());//This line MUST come before the other ones for this purpose as otherwise the others might be applied to the wrong context!
...
void Display::prepare_image(){
   ...
   //Determining colouring and shading:
      ...
      for(unsigned int i=0;i<pointvector->size();i++){//Find the maximum and minimum values from the (*pointvector):
         if(maxz<(*pointvector)[i]->getmaxZ())maxz = (*pointvector)[i]->getmaxZ();
         if(minz>(*pointvector)[i]->getminZ())minz = (*pointvector)[i]->getminZ();
         if(maxintensity<(*pointvector)[i]->getmaxintensity())maxintensity = (*pointvector)[i]->getmaxintensity();
         if(minintensity>(*pointvector)[i]->getminintensity())minintensity = (*pointvector)[i]->getminintensity();
      }
   ...
   //OpenGL setup:
      ...
      resetview();
   delete pointvector;
   returntostart();//Set up initial view and draw it.
}
