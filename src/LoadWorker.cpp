...
/*
 ...
 ==================================
 */
LoadWorker::LoadWorker(FileOpener* fo, int point_offset, std::vector<std::string> filenames, bool create_new, bool usearea, int resolutiondepth,
                       int resolutionbase, int bucketlevels, int bucketlimit, int cachelimit, bool default_scale_factors, double scale_factor[3],
                       std::string ascii_code, SelectionBox fence, PointFilter pf, std::string cache_path) :
      Worker(), fileopener(fo), point_offset(point_offset), filenames(filenames), create_new_quadtree(create_new), usearea(usearea),
      resolutiondepth(resolutiondepth), resolutionbase(resolutionbase), bucketLevels(bucketlevels), bucketlimit(bucketlimit), cachelimit(cachelimit),
      use_default_scale_factors(default_scale_factors), ascii_code(ascii_code), fence(fence), point_filter(pf), cache_path(cache_path)
{
      ...
      for(int i = 0; i < num_files; ++i)
      {
         ...
         if(filename != "")
         {
            ...
            if((i == 0 && (create_new_quadtree || !fileopener->get_loadedanyfiles())) || fileopener->get_lidardata() == NULL)
            {
               ...
               {
                  qt = new Quadtree(fence.get_min_x(), fence.get_min_y(), fence.get_max_x(), fence.get_max_y(), bucketlimit, cachelimit, bucketLevels,
...
std::string LoadWorker::get_utm_zone()
{
   ...
   } else
   {
      if(reader->header.vlr_geo_keys)
      {
         ...
         gpc.set_projection_from_geo_keys(reader->header.vlr_geo_keys[0].number_of_keys, (GeoProjectionGeoKeys*) reader->header.vlr_geo_key_entries,
                                          reader->header.vlr_geo_ascii_params, reader->header.vlr_geo_double_params);
