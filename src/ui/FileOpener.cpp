...
FileOpener::FileOpener(TwoDeeOverview *tdo, Profile *prof, const Glib::RefPtr<Gtk::Builder>& builder, AdvancedOptionsWindow *aow, FileSaver *fs,
      ...
      lidardata(NULL), tdo(tdo), prof(prof), tdow(tdow), aow(aow), fs(fs), ald(ald), eventboxtdo(eventboxtdo), eventboxprof(eventboxprof),
      bucketlimit(bucketlimit), loadworker(NULL), num_files_loading(0), newQuadtree(true), loadedanyfiles(false)
...
void FileOpener::on_filechooserdialogresponse(int response_id)
{
   ...
   } else if(response_id == 1 || response_id == 2)
   {
      ...
      // Create LoadWorker
      loadworker = new LoadWorker(this, pointskipselect->get_value_as_int(), names, create_new_quadtree, fenceusecheck->get_active(),
                                  resdepthselect->get_value_as_int(), resbaseselect->get_value_as_int(), bucketlevels, bucketlimit, cachelimit,
                                  btnUseDefault->get_active(), scale_factor, asciicodeentry->get_text(), fence, ald->get_point_filter(),
