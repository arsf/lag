...
void AdvancedOptionsWindow::
on_heightmaxselect_changed() {
   heightminselect->set_range(tdo->getrminz(),
                              heightmaxselect->get_value()-0.01);
   ...
   heightscrollbar->set_increments(1, heightmaxselect->get_value() - 
                                      heightminselect->get_value());
...
on_heightscrollbar_scrolled(Gtk::ScrollType scroll,double new_value) {
   ...
   heightminselect->set_range(tdo->getrminz(),new_value + 
                    heightscrollbar->get_adjustment()->get_page_size()-0.01);
   heightmaxselect->set_value(new_value + 
                    heightscrollbar->get_adjustment()->get_page_size());
...
void AdvancedOptionsWindow::
on_intensitymaxselect_changed(){
   intensityminselect->set_range(tdo->getrminintensity(),
                                 intensitymaxselect->get_value()-1);
   //Want the scrollbar's slider length to correlate with the range we define.
   intensityscrollbar->get_adjustment()->
                       set_page_size(intensitymaxselect->get_value() - 
                                     intensityminselect->get_value());
   intensityscrollbar->set_increments(1, intensitymaxselect->get_value() - 
                                      intensityminselect->get_value());
...
void AdvancedOptionsWindow::
on_intensityminselect_changed() {
   intensitymaxselect->set_range(intensityminselect->
                       get_value()+1,tdo->getrmaxintensity());
   // Value for the scrollbar only changed here as upper value is defined by 
   // this value and the page size.
...
on_intensityscrollbar_scrolled(Gtk::ScrollType scroll,
   ...
   //Set the new ranges first so that the old ranges cannot clamp the new values.
   intensitymaxselect->set_range(intensityminselect->get_value()+1,
                                 tdo->getrmaxintensity());
   intensityminselect->set_range(tdo->getrminintensity(),
                                 intensitymaxselect->get_value()-1);
   intensitymaxselect->set_value(new_value + 
                       intensityscrollbar->get_adjustment()->get_page_size());
...
on_drawingresetbutton_clicked(){
   ...
   heightscrollbar->get_adjustment()->set_page_size(tdo->getrmaxz() -
                                                    tdo->getrminz());
   heightscrollbar->set_increments(0.01,tdo->getrmaxz()-
                                   tdo->getrminz());
   heightoffsetselect->set_value(0);
   heightfloorselect->set_value(0);
   intensitymaxselect->set_range(tdo->getrminintensity()+1,
                                 tdo->getrmaxintensity());
   intensitymaxselect->set_value(tdo->getrmaxintensity());
   intensityminselect->set_range(tdo->getrminintensity(),
                                 tdo->getrmaxintensity()-1);
   intensityminselect->set_value(tdo->getrminintensity());
   intensityscrollbar->set_range(tdo->getrminintensity(),
                                 tdo->getrmaxintensity());
   intensityscrollbar->set_value(tdo->getrminintensity());
   intensityscrollbar->get_adjustment()->set_page_size(tdo->getrmaxintensity()-
                                                       tdo->getrminintensity());
   intensityscrollbar->set_increments(1,tdo->getrmaxintensity() -
                                      tdo->getrminintensity());
