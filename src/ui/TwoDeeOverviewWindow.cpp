...
/*
 ...
 ==================================
 */
TwoDeeOverviewWindow::TwoDeeOverviewWindow(TwoDeeOverview *tdo, AdvancedOptionsWindow *aow, FileSaver *fs, Gtk::Window *tdowin, Gtk::Window *profilewindow,
                                           const Glib::RefPtr<Gtk::Builder>& builder, Gtk::EventBox *eventboxtdo, ProfileWindow *profwin) :
      tdo(tdo), profwin(profwin), eventboxtdo(eventboxtdo), tdowin(tdowin), profilewindow(profilewindow), aow(aow), fs(fs), drawwhentoggled(true)
{
   ...
   showlegendcheck->set_inconsistent(
         !colourbyintensitymenu->get_active() && !colourbyheightmenu->get_active() && !colourbyclassificationmenu->get_active()
...
void TwoDeeOverviewWindow::on_quadtreemenu_activated()
{
   ...
   } else
   {
      ...
      } else
      {
         ...
         << (*pointvector)[0]->get_cache_used() << "\n" << "Size in memory: " << "\t" << (*pointvector)[0]->get_cache_used() * sizeof(LidarPoint) / 1048576
