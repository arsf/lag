...
TwoDeeOverview::TwoDeeOverview(const Glib::RefPtr<const Gdk::GL::Config>& config, int bucketlimit, Gtk::Label *rulerlabel) :
      LagDisplay(config, bucketlimit), drawnsofarminx(0), drawnsofarminy(0), drawnsofarmaxx(1), drawnsofarmaxy(1), resolutionbase(1), resolutiondepth(1),
      numbuckets(0), raiseline(false), linetoraise(0), drawnsinceload(false), reversez(false), showlegend(false), showdistancescale(false),
      drawneverything(false), pointcount(0), vertices(NULL), colours(NULL), tdoDisplayNoise(true), rulerlabel(rulerlabel), profiling(false), showprofile(false),
      fencing(false), showfence(false), rulerwidth(2), rulering(false), heightenNonC(false), heightenGround(false), heightenLowVeg(false),
      heightenMedVeg(false), heightenHighVeg(false), heightenBuildings(false), heightenNoise(false), heightenMass(false), heightenWater(false),
      heightenOverlap(false), heightenUndefined(false), panningRefresh(1), slicing(false), latlong(false), superzoom(false), draw_pointinfo(false)
{
   ...
   //Events and signals:
   add_events(
         Gdk::SCROLL_MASK | Gdk::BUTTON1_MOTION_MASK | Gdk::BUTTON2_MOTION_MASK | Gdk::BUTTON3_MOTION_MASK | Gdk::BUTTON_PRESS_MASK | Gdk::BUTTON_RELEASE_MASK);
      ...
      if(drawnbucketsarray[i] == false && (!cachedonly || buckets[i]->getIncacheList()[resolutionindex] == cachedonly))
      {
         ...
         for(int j = 0; j < buckets[i]->getNumberOfPoints(resolutionindex); ++j)
         {
            if(tdoDisplayNoise || (!tdoDisplayNoise && buckets[i]->getPoint(j, resolutionindex).getClassification() != 7))
            {
               ...
               if(heightenNonC || heightenGround || heightenLowVeg || heightenMedVeg || heightenHighVeg || heightenBuildings || heightenNoise || heightenMass
                  || heightenWater || heightenOverlap || heightenUndefined)
               {
                  classification = buckets[i]->getPoint(j, resolutionindex).getClassification();
...
bool TwoDeeOverview::drawbuckets(PointBucket** buckets, int numbuckets)
{
   ...
   if(!drawneverything || (((drawnsofarmaxx - centreSafe.getX() > endx && centre.getX() - centreSafe.getX() > 0)
...
void TwoDeeOverview::resetview()
{
   ...
   glOrtho(-pixelsToImageUnits(get_width() / 2), +pixelsToImageUnits(get_width() / 2), -pixelsToImageUnits(get_height() / 2),
           +pixelsToImageUnits(get_height() / 2), -5 * altitude, -5 * depth);
   ...
   if(gotdata)
   {
      ...
      if(anypoint)
      {
         ...
         string pointstring = "X: " + x.str() + ", Y: " + y.str() + ", Z: " + z.str() + ", Time: " + time.str() + ",\n" + "Intensity: " + intensity.str()
                              + ", Classification: " + classification.str() + ",\n" + "Flightline: " + flightline + " (" + flightlinenumber.str()
