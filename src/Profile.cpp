...
Profile::Profile(const Glib::RefPtr<const Gdk::GL::Config>& config, int bucketlimit, Gtk::Label *rulerlabel) :
      LagDisplay(config, bucketlimit), rulerlabel(rulerlabel), profps(0), drawpoints(true), drawmovingaverage(false), imageexists(false), slanted(true),
      slantwidth(5), showheightscale(false), totnumpoints(0), mavrgrange(5), linez(NULL), previewdetailmod(0.3), flightlinepoints(NULL), samplemaxz(0),
      sampleminz(0), viewer(Point(0, 0, 0)), width(0), rulering(false), rulerwidth(3), fencing(false), hideProfNoise(false), isProcessingFence(false),
      vertices(NULL), colours(NULL), classificationsHappening(0)
{
...
void Profile::resetview()
{
   ...
   // much room.
   glOrtho(-pixelsToImageUnits(get_width() / 2), +pixelsToImageUnits(get_width() / 2), -pixelsToImageUnits(get_height() / 2),
...
bool Profile::loadprofile(vector<double> profxs, vector<double> profys, int profps)
{
   ...
   equal(this->profxs.begin(), this->profxs.end(), profxs.begin()) && equal(this->profys.begin(), this->profys.end(), profys.begin()))
   ...
   for(int i = 0; i < bucket->getNumberOfPoints(0); ++i)
   {
      ...
      if(correctpoints[i])
      {
         ...
         { //For every edge:
            ...
            if((zs[j] < bucket->getPoint(i, 0).getZ() && zs[lastcorner] >= bucket->getPoint(i, 0).getZ()) || (zs[lastcorner] < bucket->getPoint(i, 0).getZ()
                  && zs[j] >= bucket->getPoint(i, 0).getZ()))
            {
               ...
               pnt.move(xs[j] + ((bucket->getPoint(i, 0).getZ() - zs[j]) / (zs[lastcorner] - zs[j])) * (xs[lastcorner] - xs[j]),
                        ys[j] + ((bucket->getPoint(i, 0).getZ() - zs[j]) / (zs[lastcorner] - zs[j])) * (ys[lastcorner] - ys[j]), 0);
...
void Profile::makefencebox(Point fenceStart, Point fenceEnd, Colour c)
{
   ...
   if(slanted)
   {
      ...
      glVertex3d(fenceStart.getX() - (slantwidth / 2) * deltazratio * breadth / horiz - centre.getX(),
                 fenceStart.getY() - (slantwidth / 2) * deltazratio * height / horiz - centre.getY(),
                 fenceStart.getZ() + (slantwidth / 2) * horizratio - centre.getZ());
      glVertex3d(fenceStart.getX() + (slantwidth / 2) * deltazratio * breadth / horiz - centre.getX(),
                 fenceStart.getY() + (slantwidth / 2) * deltazratio * height / horiz - centre.getY(),
                 fenceStart.getZ() - (slantwidth / 2) * horizratio - centre.getZ());
      glVertex3d(fenceEnd.getX() + (slantwidth / 2) * deltazratio * breadth / horiz - centre.getX(),
                 fenceEnd.getY() + (slantwidth / 2) * deltazratio * height / horiz - centre.getY(),
                 fenceEnd.getZ() - (slantwidth / 2) * horizratio - centre.getZ());
      glVertex3d(fenceEnd.getX() - (slantwidth / 2) * deltazratio * breadth / horiz - centre.getX(),
                 fenceEnd.getY() - (slantwidth / 2) * deltazratio * height / horiz - centre.getY(),
                 fenceEnd.getZ() + (slantwidth / 2) * horizratio - centre.getZ());
      glEnd();
...
bool Profile::on_ruler_start(GdkEventButton* event)
{
   if(event->button == 1)
   {
      ...
      rulerEnd.move(centre.getX() + viewer.getX() + hypotenuse * breadth / length, centre.getY() + viewer.getY() + hypotenuse * height / length,
