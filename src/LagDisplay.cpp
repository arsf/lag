...
LagDisplay::LagDisplay(const Glib::RefPtr<const Gdk::GL::Config>& config, int bucketlimit) :
      Gtk::GL::DrawingArea(config), zoomlevel(1), zoompower(0.5), lidardata(NULL), bucketlimit(bucketlimit), maindetailmod(0), pointsize(1), ratio(1.0),
      brightnessBy(brightnessByIntensity), colourBy(colourByFlightline), cbmaxz(0), cbminz(0), cbmaxintensity(0), cbminintensity(0), zoffset(0), zfloor(0),
      intensityoffset(0), intensityfloor(0), rmaxz(0), rminz(0), rmaxintensity(0), rminintensity(0), colourheightarray(NULL), brightnessheightarray(NULL),
      brightnessintensityarray(NULL), red(0.0), green(0.0), blue(0.0), alpha(0.0), interrupt_drawing(false), GL_data_impede(false), GL_control_impede(false),
      global_pointbucket_mutex(NULL), drawing_thread(NULL)
{
}
...
{
   return Colour(colourheightarray[3 * int(10 * (height - rminz))], colourheightarray[3 * int(10 * (height - rminz)) + 1],
