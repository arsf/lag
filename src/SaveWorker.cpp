...
/*
 ...
 ==================================
 */
SaveWorker::SaveWorker(FileSaver* fs, std::string filename, std::string filein, int flightline, std::string parse_string, bool use_latlong,
                       bool use_default_scalefactor, double scale_factor[3], Glib::Mutex* pointbucket_mutex) :
      Worker(), filesaver(fs), filename(filename), source_filename(filein), flightline_number(flightline), parse_string(parse_string),
      latlong_output(use_latlong), latlong_input(false), use_default_scalefactor(use_default_scalefactor), reader(0), writer(0), reproject_quantizer(0),
      saved_quantizer(0), pointbucket_mutex(pointbucket_mutex)
{
