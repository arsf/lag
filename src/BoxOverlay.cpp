...
BoxOverlay::BoxOverlay(Gtk::Label *label, Colour majorc, Colour minorc) :
      directional(false), orthogonalshape(false), slantedshape(true), label(label), majorcolour(majorc), minorcolour(minorc), slantwidth(30), ratio(1),
      zoomlevel(1)
...
{
   // 0 is set to the centre before scaling and adding the centre coordinate.
   end.move(centre.getX() + ((endPoint.getX() - areawidth / 2) * ratio / zoomlevel), centre.getY() - ((endPoint.getY() - areaheight / 2) * ratio / zoomlevel),
            0);
   drawinfo();
}
...
void BoxOverlay::makeboundaries()
{
   if(slantedshape)
   {
      ...
      theBox = SelectionBox(start.getX() - (slantwidth / 2) * height / length, start.getY() + (slantwidth / 2) * breadth / length, 0.0,
                            start.getX() + (slantwidth / 2) * height / length, start.getY() - (slantwidth / 2) * breadth / length, 0.0,
                            end.getX() + (slantwidth / 2) * height / length, end.getY() - (slantwidth / 2) * breadth / length, 0.0,
                            end.getX() - (slantwidth / 2) * height / length, end.getY() + (slantwidth / 2) * breadth / length, 0.0);
...
void BoxOverlay::makebox(double rmaxz)
{
   ...
   if(slantedshape)
   {
      ...
      glVertex3d(start.getX() - (slantwidth / 2) * height / length - centre.getX(), start.getY() + (slantwidth / 2) * breadth / length - centre.getY(),
                 altitude);
