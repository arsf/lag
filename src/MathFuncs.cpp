   ...
   if(!hideNoise)
   {
      for(int i = 0; i < points->getNumberOfPoints(0); ++i)
      {
         ...
         for(currentcorner = 0; currentcorner < numberofcorners; ++currentcorner)
         {
            ...
            if((ys[currentcorner] < points->getPoint(i, 0).getY() && ys[lastcorner] >= points->getPoint(i, 0).getY()) || (ys[lastcorner]
                  < points->getPoint(i, 0).getY()
                                                                                                                          && ys[currentcorner] >= points
                                                                                                                                ->getPoint(i, 0).getY()))
            {
               // If the point is to the right of the line defined by the corners
               // (and segmented by the above if statement), i.e. the edge:
               if(xs[currentcorner] + ((points->getPoint(i, 0).getY() - ys[currentcorner]) / (ys[lastcorner] - ys[currentcorner]))
