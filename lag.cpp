...
int testfilename(int argc, char *argv[], bool start, bool usearea)
{
    ...
    {//Attempt to get real files.
        ...
        {//... in the situation where there is a value of zero, check the string to see whether this is because of there being a zero value or because of there not being an integer. If the value in the string is not zero:
        ...
        {//We start after the executable path and the point offset.
            ...
            if (filename != "")
            {
                ...
                {//For las files:
                    ...
                    {//If refreshing (or from command-line) use first filename to make quadtree...
                        if (usearea)
                        {//If using the fence:
                            if (lidardata != NULL)
                            {
                    ...
                    {//... but for all other situations add to it.
                        if (usearea)
                        {//If using the fence:
                            double minX, minY, maxX, maxY;
                            get_area(minX, minY, maxX, maxY);
                ...
                {//For ASCII files (only works through GUI... Must get it to work for command-line at some point:
    ...
    if (loadedanyfiles)
    {//If drawing areas are already visible, prepare the new images and draw them.
        tdo->prepare_image();
        tdo->drawviewable(1);
        prof->prepare_image();
        prof->drawviewable(1);
    ...
    {//Otherwise, pack them into the vboxes and then show them, which will do as the above block does.
...
void on_filechooserdialogresponse(int response_id)
{
    ...
    else if (response_id == 1 || response_id == 2)
    {
        ...
        {//Until the last iterator is reached, insert the contents of the iterators into argv.
...
int main(int argc, char** argv)
{
    ...
    loaderrorstream = new ostringstream();
    lidardata = NULL; // new quadtree(0,0,1,1,bucketlimit,cachelimit,loaderrorstream);//Create quadtree now so that it can be deleted later.
