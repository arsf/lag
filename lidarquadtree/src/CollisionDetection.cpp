...
// this method takes the forumula for 4 lines and checks if the x and y 
// arguments are within them
bool kobbPoint(double m1, double c1, double m2, double c2, double m3, 
               double c3, double m4, double c4, double x, double y)
{
   // find the point where an imaginary vertical line through the x value 
   // intersects with each line
   double intersec1 = m1 * x + c1;
   double intersec2 = m2 * x + c2;
   double intersec3 = m3 * x + c3;
   double intersec4 = m4 * x + c4;

   // if the xy point is between the intersections with the oposite sides of the 
   // rectangle the
   // point is within the rectangle (think about it, it works)
   double err = 0.000001;
   if (((intersec1 - y + err) > 0 && (intersec3 - y - err) < 0) || 
       ((intersec1 - y - err) < 0 && (intersec3 - y + err) > 0))
   {
      if (((intersec2 - y + err) > 0 && (intersec4 - y - err) < 0) || 
          ((intersec2 - y - err) < 0 && (intersec4 - y + err) > 0))
      {
         return true;
      }
   }
...
// check wikipedia for more infor :P
bool triangleTest(double *Xs, double *Ys, int size, double px, double py)
{
   double sign = 0.5 * (Xs[0] * py - Ys[0] * px - Xs[size - 1] * py + 
                        Ys[size - 1] * px + Xs[size - 1] * Ys[0] + 
                        Ys[size - 1] * Xs[0]);
   for (int k = 0; k < size; k++)
   {
      double test = 0.5 * (Xs[k + 1] * py - Ys[k + 1] * px - Xs[k] * py + 
                    Ys[k] * px + Xs[k] * Ys[k + 1] + Ys[k] * Xs[k + 1]);
...
// check wikipedia for more infor :P
bool vectorTest(vector<double> Xs, vector<double> Ys, int size, double px, double py)
{
   bool in = false;
   int j = size - 1;
   for (int k = 0; k < size; k++)
...
// function to detect collision between an axis orientated rectangle and 
// a rotated rectangle
bool aoRectangleNaoRectangle(double minX, double minY, double maxX, 
                             double maxY, vector<double> Xs, vector<double> Ys, int size)
{

   // find a simple bounding box for the non axis orientated area,
   double largestX = Xs[0];
   double largestY = Ys[0];
   double smallestX = Xs[0];
   ...
   if (largestX < minX || smallestX > maxX || largestY < minY || 
       smallestY > maxY)
   {
      return false;
   }
...
// this method uses the axis seperation test to determine if two convex 
// polygons collide the two pairs of arrays it takes as argurments represent 
// the polgons and the points within them must be sequential (so that drawing 
// a line from 1 to 2 to 3 to 1 forms a polygon
bool axisSeperationTest(vector<double> poly1Xs, vector<double> poly1Ys, int poly1size,
                        vector<double> poly2Xs, vector<double> poly2Ys, int poly2size)
{
   ...
   for (int k = 0; k < poly1size; k++)
   {
      ...
      {
         // project the points onto the perpendicular vector and record the min 
         // and max positions
      ...
      // if the min max don't overlap when projected on this plane the objects 
      // don't collide
   ...
   for (int k = 0; k < poly2size; k++)
   {
      ...
      // if the min max don't overlap when projected on this vector so the objects 
      // don't collide
      if (min1 > max2 || max1 < min2)
      {
         return false;
